...
function textureUrl(hash) {
  return `https://texture.namemc.com/${hash[0]}${hash[1]}/${hash[2]}${
      hash[3]}/${hash}.png`;
}
...
exports.userStats = (identifier) => new Promise((resolve, reject) => {
  ...
  axios.get(url)
      .then((response) => {
        ...
        $('.text-lg-left').find('a').each((i, v) => {
          const acc_type =
              $(v).find('img').attr('src').split('/').pop().split('.')[0];
        ...
        data.skins = {skin_ids : [], texture_urls : []};
        ...
        for (let index = 0; index < data.skins.skin_ids.length; index += 1) {
          ...
          data.skins.texture_urls.push(
              `https://namemc.com/texture/${element}.png`);
        }

        data.optifine_cape = $('.cape-2d').attr('data-cape-hash');
  ...
  axios.get(url)
      .then((resp) => {
        ...
        if (time_attr === undefined) {
          ...
          resolve(
              {droptime : Date.parse(time_attr.replace('.000', '')) / 1000});
        }
      })
      .catch((err) => {
        console.log(err);
...
exports.upcoming = (length_op = '', length = '', lang = '', searches = '') =>
    new Promise((resolve, reject) => {
      ...
      const url = `https://namemc.com/minecraft-names?sort=&length_op=${
          operator}&length=${length}&lang=${lang}&searches=${searches}`;
      // console.log(url);
      axios.get(url)
          .then((resp) => {
            const ret = [];
            ...
            $('.p-0').find('div').each((i, v) => {
              ...
              if (name !== undefined) {
                ...
                if (!already_checked.includes(name)) {
                  ...
                  to_push.droptime =
                      Date.parse($(v).find('time').attr('datetime')) / 1000;

                  to_push.searches =
                      $(v).find('div .col-auto.col-lg.order-lg-3').text();
...
exports.searches = (username) => new Promise((resolve, reject) => {
  ...
  axios.get(url)
      .then((resp) => {
        const $ = cheerio.load(resp.data);
        const searches = parseInt($('.tabular').text().split(' ')[0], 10);
        resolve(searches);
      })
      .catch((err) => {
        console.log(err);
...
exports.skinHash = (hash) => new Promise((resolve) => {
  resolve(`https://texture.namemc.com/${hash[0]}${hash[1]}/${hash[2]}${
      hash[3]}/${hash}.png`);
});
